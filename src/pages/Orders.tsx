...
export function Orders() {
  ...
  const load = async () => {
    ...
    if (loadError) setError('No pudimos cargar los pedidos.')
  ...
  return (
    <div className="page-stack">
      ...
      <div className="toolbar">
        ...
        <div className="button-row"><select value={status} onChange={e=>setStatus(e.target.value as StatusFilter)}><option value="all">Todos los estados</option>{Object.entries(ORDER_STATUS_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select value={payment} onChange={e=>setPayment(e.target.value as PaymentFilter)}><option value="all">Todos los pagos</option>{Object.entries(PAYMENT_STATUS_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>
      </div>
      {loading ? <Loading/> : error ? <div className="empty-state"><h2>No pudimos cargar los pedidos</h2><p>{error}</p><button className="primary-button" onClick={()=>void load()}>Reintentar</button></div> : filtered.length === 0 ? <div className="empty-state"><h2>{orders.length ? 'No hay resultados' : 'No hay pedidos'}</h2><p>{orders.length ? 'Prueba otra búsqueda o filtro.' : 'Crea el primer pedido cuando tengas un cliente registrado.'}</p></div> : (
        <div className="table-wrap"><table><thead><tr><th>Código</th><th>Cliente</th><th>Cantidad</th><th>Estado</th><th>Pago</th><th>Total</th><th>Fecha</th></tr></thead><tbody>{filtered.map(order => <tr key={order.id}><td><Link className="linkish" to={`/admin/pedidos/${order.id}`}>{order.order_code}</Link></td><td>{order.customer ? <Link className="linkish" to={`/admin/clientes/${order.customer.id}`}>{customerDisplayName(order.customer)}</Link> : 'Cliente no disponible'}</td><td>{order.quantity}</td><td><span className={`status-pill ${order.status === 'cancelled' ? 'inactive' : 'active'}`}>{ORDER_STATUS_LABELS[order.status]}</span></td><td>{PAYMENT_STATUS_LABELS[order.payment_status]}</td><td>{money(order.total, order.currency)}</td><td>{new Date(order.created_at).toLocaleDateString('es-PE')}</td></tr>)}</tbody></table></div>
      )}
    </div>
  )
}
