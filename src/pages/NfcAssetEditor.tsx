...
export function NfcAssetEditor() {
  ...
  const load = async () => {
    ...
    setLoading(true)
    setError('')
    const [assetRes, ordersRes, itemsRes, cardsRes] = await Promise.all([
    ...
    if (assetRes.error) setError('No pudimos cargar este activo NFC.')
    else {
  ...
  const changeStatus = async (next: NfcAssetStatus) => {
    ...
    if (updateError) setError('No se pudo cambiar el estado. Revisa UID, pedido, tarjeta, transición y permisos.')
    else await load()
    setSaving(false)
  }
  ...
  return (
    <div className="page-stack editor-page">
      <header className="page-header">
        <div><Link className="back-link" to="/admin/inventario-nfc"><ArrowLeft size={16}/> Inventario NFC</Link><h1>{asset.asset_code}</h1><p>{NFC_CHIP_LABELS[asset.chip_type]} · {NFC_STATUS_LABELS[asset.status]}</p></div>
        <div className="button-row">{transitions.map(next => <button key={next} className={['defective','lost','retired'].includes(next) ? 'ghost-button' : 'primary-button'} disabled={saving} onClick={()=>void changeStatus(next)}>{NFC_STATUS_LABELS[next]}</button>)}</div>
      </header>
      ...
      <section className="panel form-section">
        ...
        <div className="grid-3">
          ...
          <label className="field"><span>UID físico</span><input placeholder="04A1B2C3D4E5F6" value={uid} onChange={e=>setUid(e.target.value)}/><small>Se normaliza a hexadecimal. Nunca reemplaza public_id.</small></label>
      ...
      <section className="panel form-section"><h2>Notas y fechas</h2><label className="field"><span>Notas</span><textarea rows={4} value={notes} onChange={e=>setNotes(e.target.value)}/></label><div className="grid-3"><div><strong>Reservada</strong><p>{asset.reserved_at ? new Date(asset.reserved_at).toLocaleString('es-PE') : '—'}</p></div><div><strong>Programada</strong><p>{asset.programmed_at ? new Date(asset.programmed_at).toLocaleString('es-PE') : '—'}</p></div><div><strong>Entregada</strong><p>{asset.delivered_at ? new Date(asset.delivered_at).toLocaleString('es-PE') : '—'}</p></div></div></section>
